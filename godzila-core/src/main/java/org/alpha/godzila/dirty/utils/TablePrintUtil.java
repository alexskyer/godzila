 ...
 */
package org.alpha.godzila.dirty.utils;
...
public class TablePrintUtil {
     ...
     * 打印数据表格
     *
     * @param dirtyDataEntry dirtyDataEntry
     */
    public static void printTable(DirtyDataEntry dirtyDataEntry) {
        List<String[]> data = new ArrayList<>(2);

        Class<? extends DirtyDataEntry> entryClass = dirtyDataEntry.getClass();
        Field[] declaredFields = entryClass.getDeclaredFields();
        String[] recordStr = new String[declaredFields.length];
        ...
        for (int i = 0; i < declaredFields.length; i++) {
            try {
                ...
                recordStr[i] = String.valueOf(declaredFields[i].get(dirtyDataEntry));
    ...
    private int[] getColLengths() {
        ...
        for (int x = 0; x < result.length; x++) {
            ...
            for (String[] datum : data) {
                int len = getStringCharLength(datum[x]);
                if (len > max) {
                    max = len;
                }
            }
